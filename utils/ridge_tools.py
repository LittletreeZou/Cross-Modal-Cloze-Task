'''
Taken from https://github.com/mtoneva/brain_language_nlp
'''
...
def ridge(X,Y,lmbda):
    # modified by sxz, we do not want to regularize the bias
    I = np.eye(X.shape[1])
    I[-1, -1] = 0   # 消除对bias惩罚
    return np.dot(inv(X.T.dot(X)+lmbda*I), X.T.dot(Y))
...
def ridge_by_lambda(X, Y, Xval, Yval, lambdas=np.array([0.1,1,10,100,1000])):
    # select model based on correlations
    corrs = np.zeros((lambdas.shape[0],Y.shape[1]))
    for idx,lmbda in enumerate(lambdas):
        weights = ridge(X,Y,lmbda)
        corrs[idx] = corr(np.dot(Xval,weights),Yval)
    return corrs
